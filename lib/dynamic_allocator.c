...
void free_block(void* va)
{
	...
	if (prev_alloc && next_alloc) {
		struct BlockElement *prevelement = (struct BlockElement*) ((char*)va - prev_size);
		...
		struct BlockElement *element = (struct BlockElement*) ((char*)va - prev_size);
		LIST_REMOVE(&freeBlocksList, element);
...
void *realloc_block_FF(void* va, uint32 new_size)
{
	...
	if (new_size == 0 && va != (void*) NULL) {
		...
		struct BlockElement *va_next = (struct BlockElement*) (va + get_block_size(va));
		...
		if (new_size + 8 > get_block_size(va)) {
			...
			if (get_block_size((va_next)) < ((new_size + 8) - get_block_size(va))) {
			...
			else if (get_block_size((void *) (va_next)) - ((new_size + 8) - get_block_size(va)) < 16)
				{
...

				{
					...
					struct BlockElement * old_block = (struct BlockElement *) (uint8*) va;
					...
					set_block_data(nextVA, ((size_of_next_block + oldsize) - (new_size + 8)), 0);
		...
		else
		{
			...
			struct BlockElement *header_next3 = (struct BlockElement*) ((uint8 *) va + size_of_new_block);
