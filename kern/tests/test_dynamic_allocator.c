...
void test_realloc_block_FF()
{
	...
	for (int i = 0; i < numOfAllocs; ++i)
	{
		for (int j = 0; j < allocCntPerSize; ++j)
		{
			...
			expectedVA = (curVA + sizeOfMetaData/2); // after header (VA)
	...
	*(endVAs[idx]) = idx ;
	...
	for (int i = 0; i < idx; ++i) {
		if (*(startVAs[i]) != i || *(midVAs[i]) != i || *(endVAs[i]) != i)
			cprintf("failed to save same data for test sending existing va and same size of that address");

	}

	if (is_correct) {
		cprintf("same data test ran successfully\n");
	...
	is_correct = 1;
	{
		...
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.1.2: WRONG REALLOC! content of the block is not correct. Expected %d, found %d\n", blockIndex,*(startVAs[blockIndex]));
		}
	...
	is_correct = 1;
	{
		...
		{
			is_correct = 0;
			cprintf("test_realloc_block_FF #3.2.2: WRONG REALLOC! content of the block is not correct. Expected %d Found %d\n", blockIndex, *(startVAs[blockIndex]) );
		}
	...
	is_correct = 1;
	cprintf("implemented test 1: reallocate in different place relocate needed\n\n") ;
	{


		blockIndex = 2 * allocCntPerSize - 1;
		new_size = 20 + 4*kilo - sizeOfMetaData;
		expectedSize = ROUNDUP(new_size + sizeOfMetaData, 2);
		expectedVA = startVAs[1200];
		void * old_va = startVAs[blockIndex];
		void * x = NULL;

		uint32 fr = check_list_size(expectedNumOfFreeBlks);

		*(startVAs[blockIndex]) = blockIndex;
		...
		*(endVAs[blockIndex]) = blockIndex;
		...
		if (is_correct) {
			cprintf("Implemented test 1 ran successfully \n");

		}
	}

		is_correct = 1;
		cprintf("implemented test 2: new size greater than all free blocks\n\n") ;
		{
			blockIndex = 1 * allocCntPerSize - 1;
			new_size = 8*kilo;
			expectedSize = ROUNDUP(new_size + sizeOfMetaData, 2);
			...
			if (va == NULL) {
				cprintf("Implemented test 2 ran successfully \n");

			}else{
				cprintf("	ERROR: implemented test 2: new size greater than all free blocks\n\n") ;
	...
	cprintf("implemented test 3 decrease size with next block free\n\n") ;
	...
	is_correct = 1;
	{
		...
		*(endVAs[blockIndex]) = blockIndex;
		...
		if (check_block(va, expectedVA, expectedSize, 1) == 0) {
			is_correct = 0;
			cprintf("VA not equal to expectedVA or header and footer incorrect\n");
		}
		struct BlockElement *newBlkAddr = (struct BlockElement *) (va + new_size + 2 * sizeof(int));
		cprintf("\nrealloc Test: newBlkAddr @va %x\n", newBlkAddr);
		expectedSize = 1 * kilo + allocSizes[1];
		if (check_block(newBlkAddr, newBlkAddr, expectedSize, 0) == 0) {
			is_correct = 0;
			cprintf("Free block expected size not equal actual size\n");
		}
		if (*(startVAs[blockIndex]) != blockIndex || *(midVAs[blockIndex]) != blockIndex) {
			is_correct = 0;
			cprintf("data error Expected %d found %d\n",blockIndex, *(startVAs[blockIndex]));
		}

		if (is_correct) {
			cprintf("\nimplemented test 3 ran successfully \n");
	...
	is_correct = 1;
	cprintf("implemented test 4: send existing va with the same size of it\n\n") ;

	{
		...
		va = realloc_block_FF(expectedVA, new_size);

		if (check_block(va, expectedVA, expectedSize, 1) == 0) {
			is_correct = 0;
			cprintf("\nfailed to send back same address for test sending existing va and same size of that address\n");
		}
		if (*(startVAs[blockIndex]) != blockIndex || *(midVAs[blockIndex]) != blockIndex || *(endVAs[blockIndex]) != blockIndex) {
			is_correct = 0;
			cprintf("\nfailed to send back same data for test sending existing va and same size of that address\n");
		}
		if(is_correct){
			cprintf("\nimplemented test 4 ran successfully \n") ;
