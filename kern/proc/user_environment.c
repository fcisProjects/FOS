...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
	...
	uint32* kernel_stack = kmalloc(KERNEL_STACK_SIZE);
	cprintf("done in stack after allocate stake pages \n");

		if (kernel_stack == NULL) {
			panic("Not enough kernel heap space for user kernel stack");
		}
		//cprintf("done in stack add %p\n  ",kernel_stack);

		uint32 x=kheap_physical_address((uint32)kernel_stack);
		cprintf("done in stack ph add before unmap  %p \n",x);
        //physical_table[PPN(x)].virtual_add=(uint32)NULL;

       // struct FrameInfo *free1 =to_frame_info(kheap_physical_address((uint32)kernel_stack));


		//cprintf("done in stack  ver in the table  %p \n",physical_table[PPN(x)].virtual_add);
		//cprintf("done in stack var add before unmap  %p \n",free1->va);

		//unmap_frame(ptr_user_page_directory, (uint32) kernel_stack);
		uint32 * page_table;
		int i=get_page_table(ptr_user_page_directory,(uint32)kernel_stack,&page_table);
		page_table[PTX(kernel_stack)]&=(~PERM_PRESENT);//marked not present

         //allocate_frame()
        //free1->Virtual_Address=(uint32)NULL;
//        cprintf("done in stack ph add after unmap  %p \n",x);
//        cprintf("done in stack va add after unmap and deleted %p \n",free1->va);
//
//
//		cprintf("done in stack \n");
