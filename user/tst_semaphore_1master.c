...
_main(void)
{
	...
	cprintf("Master: Running envs\n");
	sys_run_env(id1);
	sys_run_env(id2);
	sys_run_env(id3);

	cprintf("Master: Waiting for depend1 semaphore\n");
	wait_semaphore(depend1);
	wait_semaphore(depend1);
	wait_semaphore(depend1);

	cprintf("Master: All slaves have completed. Validating results\n");
